...
for label, dset, clf in product(SHORTCUTS['groups']['EZ'], DSETS, CLFs):
    savedir = '{}/{}/{}/{}'.format(basedir, clf, dset, label)
    ...
    data = remove_baseline(data, baseline, .5)
    dataPFC = remove_baseline(dataPFC, baseline, .5)
    dataSTR = remove_baseline(dataSTR, baseline, .5)

    # In each compare first 50 with next 50.
    trials = data.trial.unique()
    sep1 = trials[ntrials_init_vs_after]
    sep2 = trials[2*ntrials_init_vs_after]
    for df, area in zip([data, dataPFC, dataSTR], ['both', 'PFC', 'STR']):
        ...
        res = shuffle_val_predict(clf, [ini_df, end_df], ['first50', 'next50'],
                                    n_splits=30, feature_scaling='standard',
